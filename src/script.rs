...
impl Script {
    pub async fn execute(&self, tpl: &Tpl, cwd : &Path) -> Result<()> {
        ...
        let name = self.name.clone().map(|s|format!("'{}'",s)).unwrap_or("".to_string());
        log_info!("Script","running script {}",name);
        let file = format!("{}.{}",self.name.clone().unwrap_or(Uuid::new_v4().to_string()),self.kind.to_string());
        let mut argv = self.interpreter.clone()
            .or(self.kind.interpreter())
            .expect(&format!("unable to determine interpreter for script `{}`; please specify explicitly",file));
